...
def initial_voxelize(z, init_res, after_res):
    ...
    sparse_hash, idx_query, counts = torch.unique(pc_hash, return_inverse=True, return_counts=True)
    counts = counts.int()
...
def voxel_to_point(x, z, nearest=False):
    if z.idx_query is None or z.weights is None or z.idx_query.get(x.s) is None or z.weights.get(x.s) is None:
        ...
        old_hash = F.sphash(
            torch.cat([torch.floor(z.C[:, :3] / x.s[0]).int() * x.s[0], z.C[:, -1].int().view(-1, 1)], 1), off
        )
